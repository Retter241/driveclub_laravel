...
<div class="container">
    <div class="row justify-content-center">
        <div class="col-md-8">
            <div class="card">
                <div class="card-header">Редктирование новости</div>
...
{!! Form::hidden('id', $post->id) !!}
        ...
        <div class="form-group col">
            <a href="#" data-toggle="tooltip" data-placement="bottom" title="<img class='img-responsive' src='{{ config('app.url') }}/{{ $post->img }}' width='175' height='75'  />">Картинка для превью</a><!--{?для замены загрузить новую  в поле вверху?}-->
        </div>
            <div class="form-group  col-md-6">
               {!! Form::label('meta_title', 'Заголовок') !!}  
                {!! Form::text('meta_title', $post->meta_title, ['class' => 'form-control' , 'placeholder' => 'meta_title']) !!}


           

                {!! Form::label('meta_desc', 'Описание (СЕО)') !!}  
                {!! Form::text('meta_desc', $post->meta_desc, ['class' => 'form-control' , 'placeholder' => 'meta_desc']) !!}
                ...
                {!! Form::text('title', $post->title, ['class' => 'form-control' , 'placeholder' => 'title' ,'style'=> 'display:none;']) !!}

                 {!! Form::label('alias', 'Алиас') !!}  
                {!! Form::text('alias', $post->alias, ['class' => 'form-control' , 'placeholder' => 'alias']) !!}
                </div>
 ...
 <div class="form-group ">
@php
 $sliders = json_decode($post->slider_photo, true);
@endphp
    @if(is_null($sliders))
        Slider is Empty!
    @else 
        @foreach($sliders as $slide)
             <a href="#" data-toggle="tooltip" data-placement="bottom" title="<img class='img-responsive' src='{{ config('app.url') }}/{{ $slide }}' width='175' height='75'  />">Картинка для превью</a>        
        @endforeach
        <!--{?для замены загрузить новую  в поле внизу?}  -->
    @endif
 </div>
